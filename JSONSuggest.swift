...
public class JSONSuggest{
    ...
    var saveDirectory = "/Users/andrewggoodwin/Desktop"
    ...
    func makeSuggestions(JSON:AnyObject, root:String){
        if isDebug{
            if deliveryMethod == .FileSystem && saveDirectory == ""{
                assert(false, "To save to the file system, please set the .saveDirectory property. Please try /Users/<username/Desktop")
            }
            if deliveryMethod == .FileSystem && (saveDirectory.hasPrefix("~") || saveDirectory.hasPrefix("$")){
                assert(false, "Save directory cannot contain aliases. Please try /Users/<username/Desktop")
            }
