...
public class JSONSuggest{
    ...
    var deliveryMethod:DeliveryMethod = .URL
    ...
    func writeClass(cls:SuggestClass)->String{
        ...
        if includeSerialization{
            ...
            if requiredProperties.count > 0{
                ...
                for property in requiredProperties{
                    if property.type.hasPrefix("[") && !property.isSimple(){
                        fileString += writeLine("                self.\(property.name.propertycaseString) = \(property.type.firstCapitalizedString.stringByReplacingOccurrencesOfString("[", withString: "").stringByReplacingOccurrencesOfString("]", withString: "")).fromJSONArray(\(property.name.propertycaseString))")
                    }
                    else{
    ...
    func printClass(cls:SuggestClass){
        ...
        if includeSerialization{
            ...
            if requiredProperties.count > 0{
                ...
                for property in requiredProperties{
                    if property.type.hasPrefix("[") && !property.isSimple(){
                        print("                self.\(property.name.propertycaseString) = \(property.type.firstCapitalizedString.stringByReplacingOccurrencesOfString("[", withString: "").stringByReplacingOccurrencesOfString("]", withString: "")).fromJSONArray(\(property.name.propertycaseString))")
                    }
                    else{
                        print("                self.\(property.name.propertycaseString) = \(property.name.propertycaseString)")
                    }
